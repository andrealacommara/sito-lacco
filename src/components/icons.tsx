...
var match = logoLacco.match(/<path[^>]*d="([^"]*)"/);
const logoPath = match ? match[1] : "";
match = logoSpotify.match(/<path[^>]*d="([^"]*)"/);
const spotifyPath = match ? match[1] : "";
match = logoInstagram.match(/<path[^>]*d="([^"]*)"/);
const instagramPath = match ? match[1] : "";
match = logoTikTok.match(/<path[^>]*d="([^"]*)"/);
...
export const InstagramIcon: React.FC<IconSvgProps> = ({
  ...
  return (
    <svg
      ...
      {...props}
    >
      <path d={instagramPath} fill="currentColor" />
    </svg>
  );
};
